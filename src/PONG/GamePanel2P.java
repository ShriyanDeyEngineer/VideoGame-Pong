...
import PONG.GamePanel.Ball;
import PONG.GamePanel.Paddle;
...
public class GamePanel2P extends JPanel implements ActionListener
{
	...
	boolean gameFor2P_Selected = false;
	...
	public GamePanel2P() 
	{
		...
		scoreBoard = new JLabel("Score: 0 : 0");
		scoreBoard.setForeground(Color.WHITE);
		scoreBoard.setFont(new Font("SansSerif", Font.PLAIN, 50));
		scoreBoard.setSize(350, 50);
        add(scoreBoard);
        scoreP1 = 0;
        scoreP2 = 0;
        hitScore = 0;
		...
		PongBall = new Ball(10, direction * resetPongBallSpeed_x, direction * resetPongBallSpeed_y); //Adds a ball to the game
		PongPaddle = new Paddle(15, 180, resetPongPaddleSpeed); //Adds a paddle to the game for player 1
		PongPaddle2 = new Paddle(15, 180, resetPongPaddleSpeed); //Adds a paddle to the game for player 2
		
		
		//The chunk of code encapsulated by the comment lines is vital to implementing smooth control of the paddles by utilizing
		//Press and release of key binds (flags) to remove the delay movement delay after a player holds down a key 
		//------------------------------------------------------------------------------------------------
		//Controls player 1's paddle
		getInputMap().put(KeyStroke.getKeyStroke("pressed UP"), "upPressed"); //Basically sets up a key bind/trigger and gives it a name
		getActionMap().put("upPressed", new AbstractAction() { //Basically gives the key bind a function/what to do when it is used by using the name given in a getInputMap method
			public void actionPerformed(ActionEvent e) {
				upPressed = true;
			}
		});
		getInputMap().put(KeyStroke.getKeyStroke("released UP"), "upReleased"); //This line and the next 5 lines get the paddle to stop moving after the key is released
		...
		getInputMap().put(KeyStroke.getKeyStroke("pressed DOWN"), "downPressed");
		getActionMap().put("downPressed", new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				downPressed = true;
			}
		});
		getInputMap().put(KeyStroke.getKeyStroke("released DOWN"), "downReleased");
		...
		//Controls player 2's paddle
		getInputMap().put(KeyStroke.getKeyStroke("pressed W"), "WPressed"); //Basically sets up a key bind/trigger and gives it a name
		getActionMap().put("WPressed", new AbstractAction() { //Basically gives the key bind a function/what to do when it is used by using the name given in a getInputMap method
			public void actionPerformed(ActionEvent e) {
				W_Pressed = true;
			}
		});
		getInputMap().put(KeyStroke.getKeyStroke("released W"), "WReleased"); //This line and the next 5 lines get the paddle to stop moving after the key is released
		...
		addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				resetBall();
				resetPaddle();
				scoreBoard.setLocation(getWidth() - scoreBoard.getWidth() - 30, getHeight() - scoreBoard.getHeight() - 20);
	...
	{
		if(scoreP1 < 3 && scoreP2 < 3 && canStartTimer)
		{
			PongBall.move();
			
			if(upPressed == true && PongPaddle.yPos >= 0)
				PongPaddle.yPos -= PongPaddle.moveSpeed;
			if(downPressed == true && PongPaddle.yPos <= getHeight() - PongPaddle.height)
				PongPaddle.yPos += PongPaddle.moveSpeed;
			
			if(W_Pressed == true && PongPaddle2.yPos >= 0)
				PongPaddle2.yPos -= PongPaddle2.moveSpeed;
			if(S_Pressed == true && PongPaddle2.yPos <= getHeight() - PongPaddle2.height)
				PongPaddle2.yPos += PongPaddle2.moveSpeed;
	...
	public void resetBall() //Just centers the ball at the start of the game in the middle of the screen
	{
		...
		int direction = Math.random() < 0.5 ? -1 : 1; //Randomizes the ball velocity direction (uses shorthand notation for an if/else statement)
		PongBall.yVelocity *= direction;
	}
	
	public void resetPaddle() //Just centers the paddle at the top/bottom of the screen at the start of the game
	{
		PongPaddle.xPos = getWidth() - getWidth() / 16;
		PongPaddle.yPos = (getHeight() - PongPaddle.height) / 2;
		
		PongPaddle2.xPos = getWidth() / 16;
		PongPaddle2.yPos = (getHeight() - PongPaddle2.height) / 2;
	}
	...
	public void resetGame() 
	{
		...
		scoreP1 = resetScores;
		scoreP2 = resetScores;
		hitScore = resetScores;
		
		scoreBoard.setText("Score: 0 | 0");
		scoreBoard.setLocation(getWidth() - scoreBoard.getWidth() - 30, getHeight() - scoreBoard.getHeight() - 20);
		add(scoreBoard);
		PongBall.ballColor = Color.WHITE;
		controlIncreaseGameSpeed = false;
		upPressed = false;  //Fixes the logic error where the game sometimes starts with the paddle moving without the user's input (right arrow key)
		downPressed = false;   //Fixes the logic error where the game sometimes starts with the paddle moving without the user's input (left arrow key)
		W_Pressed = false;  //Fixes the logic error where the game sometimes starts with the paddle moving without the user's input ("D" key)
		S_Pressed = false;   //Fixes the logic error where the game sometimes starts with the paddle moving without the user's input ("A" key)
		timer.stop();
	}
	...
	public class Ball //Class for the ball which handles the attributes of the ball but not for its functionality
	{
		...
		public void move()
		{
			if(xPos >= getWidth() - 2 * radius || xPos < 0)
			{
				...
				{
					scoreP1 += 1;
					scoreBoard.setText("Score: " + scoreP2 + " | " + scoreP1);
			...
			if(yPos >= getHeight() - 2 * radius || yPos < 0)
			{
				PlaySFX(1);
				yVelocity *= -1;
			}
			
			else if(xVelocity > 0 && xPos + 2 * radius >= PongPaddle.xPos && xPos <= PongPaddle.xPos + PongPaddle.width && yPos + 2 * radius >= PongPaddle.yPos && yPos <= PongPaddle.yPos + PongPaddle.height) //checks the necessary conditions for the ball to rebound of the paddle of player 1
			{
				PlaySFX(0);
				xVelocity *= -1;
				hitScore += 1;
			...
			else if(xVelocity < 0 && xPos + 2 * radius >= PongPaddle2.xPos && xPos <= PongPaddle2.xPos + PongPaddle2.width && yPos + 2 * radius >= PongPaddle2.yPos && yPos <= PongPaddle2.yPos + PongPaddle2.height) //checks the necessary conditions for the ball to rebound of the paddle of player 2
			{
				PlaySFX(0);
				xVelocity *= -1;
				hitScore += 1;
			...
			//Check the hit score to see whether or not to switch or do not switch the ball color depending on the player's current score and change the game speed
			...
			//If the players are going crazy and their hit score is off the charts, turn the ball crazy also and make it rainbow
			if(hitScore >= 110)
			{
				if(rainbowColor == 9)
				{
	...
	public class Paddle //Class for the paddle that the player controls which handles the attributes of a paddle but not for its functionality
	{
		private int xPos;
		private int yPos;
		private int width;
